...
public class AdoptionManager {
    ...
    public void processAdoption() {
        ...
        //在这里加一个登陆验证吧
        System.out.println("Enter adopter password: ");
        String password = scanner.nextLine();
        if(adoptionService.getAdopterById(adopterId).verifyPassword(password)) {
            //进来了，说明匹配上了
            adoptionService.adoptPet(petId, adopterId);
            System.out.println("Adoption processed successfully!");
        }else{
            System.out.println("Invalid adopter ID or password.");
        }
    }
